...
%             drawall = 0;
        ...
        elseif length(size(varargin{i})) == 2
            view_az = varargin{i}(1);
            view_el = varargin{i}(2);
...
if ~isempty(CfgFileName)
    ...
    if exist("view_el",'var')  % Modified by Pratik jain to make it compatible with WhiFun toolbox
        EC.lot.view_az = view_az;
        EC.lot.view_el = view_el;
    end

    if exist('draw_','var')    % Modified by Pratik jain to make it compatible with WhiFun toolbox
        EC.vol.roi.drawall = 0;
        EC.vol.roi.draw = draw_;
    end
